...
public class Level : MonoBehaviour, IWaveObserver, ILevelObserver, ICaptureBallObserver, IPurchaseObserver, IRewardedObserver, IConsentObserver, IInterstitialObserver
{
        ...
        for (var d = -deep; d <= deep; d++)
        {
            for (var j = 0; j < sq * sq; j++)
            {
                ...
                if (playerState.level == 0)
                    ball = Instantiate(gameConfig.ExtraBalls[Random.Range(0, 6)], newLab.transform);
                else if (playerState.level == 4)
                    ball = Instantiate(gameConfig.WaterDrop, newLab.transform);
                else
                ...
                if (playerState.level == 4)
                    ball.transform.localPosition = new Vector3(ball.transform.localPosition.x, ball.transform.localPosition.y, 0f);
    ...
    private void WaterPoolObjectIn()
    {
        int count = 0;
        foreach (var ball in balls)
        {
            ...
            ball.transform.position = new Vector3(100f + count, 100f + count, 100f + count);
    ...
    private void WaterPoolObjectOut()
    {
        ...
        for (var d = -deep; d <= deep; d++)
        {
            for (var j = 0; j < sq * sq; j++)
            {
                balls[count].transform.SetParent(labyrinths[activeLab].transform, true);
                balls[count].transform.localPosition = Vector3.right * (j % sq - sq / 2) * 0.1f + Vector3.down * (j / sq - sq / 2) * 0.1f + Vector3.forward * d * 0.1f;
                balls[count].transform.localPosition = new Vector3(balls[count].transform.localPosition.x, balls[count].transform.localPosition.y, 0f);
                balls[count].GetComponent<Rigidbody>().useGravity = true;
                count++;
            }
        }
        _waterPoolled = false;
